# 🏗️ API Watcher V2 - Архитектура

## 📐 Общая схема

```
┌─────────────────────────────────────────────────────────────────┐
│                        API Watcher V2                            │
│                                                                   │
│  ┌──────────────┐    ┌──────────────┐    ┌──────────────┐      │
│  │   URLs.json  │───▶│  Watcher V2  │───▶│   Database   │      │
│  └──────────────┘    └──────┬───────┘    └──────────────┘      │
│                              │                                    │
│                              ▼                                    │
│                    ┌─────────────────┐                           │
│                    │  Content Fetch  │                           │
│                    └────────┬────────┘                           │
│                             │                                     │
│              ┌──────────────┼──────────────┐                    │
│              ▼              ▼               ▼                    │
│        ┌─────────┐    ┌─────────┐    ┌─────────┐               │
│        │ Direct  │    │ZenRows  │    │ Parser  │               │
│        │ Request │    │  API    │    │ (HTML)  │               │
│        └─────────┘    └─────────┘    └─────────┘               │
│                             │                                     │
│                             ▼                                     │
│                    ┌─────────────────┐                           │
│                    │ Smart Comparator│                           │
│                    └────────┬────────┘                           │
│                             │                                     │
│              ┌──────────────┼──────────────┐                    │
│              ▼              ▼               ▼                    │
│        ┌─────────┐    ┌─────────┐    ┌─────────┐               │
│        │  Hash   │    │OpenAPI  │    │  HTML   │               │
│        │ Compare │    │Struct   │    │  Text   │               │
│        └─────────┘    └─────────┘    └─────────┘               │
│                             │                                     │
│                             ▼                                     │
│                    ┌─────────────────┐                           │
│                    │  Has Changes?   │                           │
│                    └────────┬────────┘                           │
│                             │                                     │
│                    ┌────────┴────────┐                           │
│                    │       YES       │                           │
│                    ▼                 ▼                           │
│            ┌──────────────┐   ┌──────────────┐                  │
│            │ OpenRouter   │   │  Categorize  │                  │
│            │   AI Analysis│   │   (OpenAPI)  │                  │
│            └──────┬───────┘   └──────┬───────┘                  │
│                   │                   │                           │
│                   └─────────┬─────────┘                          │
│                             ▼                                     │
│                    ┌─────────────────┐                           │
│                    │  Save Snapshot  │                           │
│                    │   + AI Summary  │                           │
│                    └────────┬────────┘                           │
│                             │                                     │
│                             ▼                                     │
│                    ┌─────────────────┐                           │
│                    │ Slack Notifier  │                           │
│                    └─────────────────┘                           │
└───────────────────────────────────────────────────────────────────┘
```

## 🔄 Workflow детально

### 1. Инициализация

```python
watcher = APIWatcherV2()
├── DatabaseManager (SQLite/PostgreSQL)
├── ZenRowsClient (если настроен)
├── OpenRouterAnalyzer (если настроен)
├── SlackNotifier (если настроен)
└── SmartComparator
```

### 2. Обработка URL

```
process_url(url, api_name, method_name)
│
├─▶ fetch_content(url)
│   ├─▶ ZenRows (если доступен)
│   │   ├─▶ Попытка 1: базовые настройки
│   │   ├─▶ Попытка 2: премиум прокси
│   │   └─▶ Попытка 3: без JS рендеринга
│   └─▶ requests (fallback)
│
├─▶ detect_content_type(url, content)
│   ├─▶ 'openapi' (если OpenAPI/Swagger)
│   ├─▶ 'json' (если валидный JSON)
│   └─▶ 'html' (по умолчанию)
│
├─▶ get_latest_snapshot(url)
│   ├─▶ Если нет → сохранить первый снэпшот
│   └─▶ Если есть → продолжить сравнение
│
└─▶ _compare_content(old, new, type)
    ├─▶ OpenAPI → _compare_openapi()
    ├─▶ JSON → _compare_json()
    └─▶ HTML → _compare_html()
```

### 3. Сравнение OpenAPI

```
_compare_openapi(old_spec, new_spec)
│
├─▶ Структурное сравнение (DeepDiff)
│   ├─▶ Игнорировать: version, servers
│   └─▶ Сравнить: paths, schemas, parameters
│
├─▶ Если изменений нет → return False
│
├─▶ Если изменения есть:
│   ├─▶ categorize_openapi_changes()
│   │   ├─▶ new_endpoints
│   │   ├─▶ removed_endpoints
│   │   ├─▶ modified_endpoints
│   │   ├─▶ schema_changes
│   │   └─▶ breaking_changes
│   │
│   ├─▶ OpenRouter AI: analyze_openapi_changes()
│   │   └─▶ Создать человекочитаемую сводку
│   │
│   ├─▶ Определить severity:
│   │   ├─▶ major (breaking changes)
│   │   ├─▶ moderate (new/removed endpoints)
│   │   └─▶ minor (остальное)
│   │
│   ├─▶ save_snapshot() в БД
│   │
│   └─▶ Slack: send_change_notification()
│
└─▶ return result
```

### 4. Сравнение HTML

```
_compare_html(old_html, new_html)
│
├─▶ Быстрая проверка по хешу
│   └─▶ Если хеши равны → return False
│
├─▶ html_to_text() для обоих версий
│   └─▶ Конвертация HTML → читаемый текст
│
├─▶ Сравнение текстов
│   └─▶ Если текст не изменился → return False
│
├─▶ OpenRouter AI: analyze_changes()
│   ├─▶ Промпт: "Определи существенные изменения"
│   ├─▶ Игнорировать: опечатки, форматирование
│   └─▶ Вернуть: has_significant_changes, summary, key_changes
│
├─▶ Если изменения незначительные:
│   ├─▶ save_snapshot(has_changes=False)
│   └─▶ return False (без уведомления)
│
├─▶ Если изменения существенные:
│   ├─▶ save_snapshot(has_changes=True, ai_summary)
│   └─▶ Slack: send_change_notification()
│
└─▶ return result
```

### 5. Еженедельный дайджест

```
send_weekly_digest()
│
├─▶ get_snapshots_with_changes(days=7)
│   └─▶ Получить все изменения за неделю из БД
│
├─▶ Сформировать список изменений
│   ├─▶ api_name
│   ├─▶ method_name
│   ├─▶ url
│   ├─▶ summary (AI-сводка)
│   └─▶ created_at
│
└─▶ Slack: send_weekly_digest(changes)
    ├─▶ Заголовок с количеством изменений
    ├─▶ Список изменений (до 10)
    └─▶ Кнопки для открытия документации
```

## 🗄️ Структура базы данных

### Таблица: snapshots

```sql
CREATE TABLE snapshots (
    id              INTEGER PRIMARY KEY AUTOINCREMENT,
    url             VARCHAR(500) NOT NULL,
    api_name        VARCHAR(200),
    method_name     VARCHAR(200),
    content_type    VARCHAR(50),      -- 'html', 'openapi', 'json'
    
    -- Контент
    raw_html        TEXT,              -- Сырой HTML
    text_content    TEXT,              -- Текстовая версия
    structured_data TEXT,              -- JSON для OpenAPI/JSON
    
    -- Метаданные
    created_at      DATETIME DEFAULT CURRENT_TIMESTAMP,
    has_changes     BOOLEAN DEFAULT 0,
    ai_summary      TEXT,              -- AI-сводка изменений
    content_hash    VARCHAR(64),       -- SHA-256 хеш
    
    -- Индексы
    INDEX idx_url (url),
    INDEX idx_created_at (created_at),
    INDEX idx_has_changes (has_changes)
);
```

### Примеры запросов

```sql
-- Последний снэпшот для URL
SELECT * FROM snapshots 
WHERE url = 'https://api.example.com/docs' 
ORDER BY created_at DESC 
LIMIT 1;

-- История изменений
SELECT api_name, method_name, created_at, ai_summary 
FROM snapshots 
WHERE url = 'https://api.example.com/docs' 
  AND has_changes = 1 
ORDER BY created_at DESC;

-- Все изменения за неделю
SELECT * FROM snapshots 
WHERE has_changes = 1 
  AND created_at >= datetime('now', '-7 days') 
ORDER BY created_at DESC;

-- Статистика по API
SELECT 
    api_name,
    COUNT(*) as total_checks,
    SUM(has_changes) as changes_count
FROM snapshots 
GROUP BY api_name;
```

## 🔌 Интеграции

### ZenRows API

```python
class ZenRowsClient:
    BASE_URL = "https://api.zenrows.com/v1/"
    
    def fetch_html(url, js_render=True, premium_proxy=False, antibot=True):
        params = {
            'apikey': api_key,
            'url': url,
            'js_render': 'true' if js_render else 'false',
            'premium_proxy': 'true' if premium_proxy else 'false',
            'antibot': 'true' if antibot else 'false'
        }
        response = requests.get(BASE_URL, params=params)
        return response.text
```

**Стратегия fallback:**
1. Базовые настройки (js_render=True, premium_proxy=False)
2. С премиум прокси (js_render=True, premium_proxy=True)
3. Без JS (js_render=False, premium_proxy=False)

### OpenRouter AI

```python
class OpenRouterAnalyzer:
    def analyze_changes(old_text, new_text):
        prompt = f"""
        Сравни два текста документации и определи:
        1. Есть ли СУЩЕСТВЕННЫЕ изменения?
        2. Краткая сводка (2-3 предложения)
        3. Ключевые изменения списком
        
        СТАРАЯ ВЕРСИЯ: {old_text[:15000]}
        НОВАЯ ВЕРСИЯ: {new_text[:15000]}
        
        Ответь в JSON:
        {{
            "has_significant_changes": true/false,
            "summary": "...",
            "key_changes": [...],
            "severity": "minor/moderate/major"
        }}
        """
        
        response = model.generate_content(prompt)
        return parse_json(response.text)
```

**Оптимизация:**
- Ограничение текста: 15000 символов (чтобы не превысить лимит токенов)
- Кэширование: AI вызывается только при обнаружении изменений
- Fallback: если AI недоступен, используется базовое сравнение

### Slack API

```python
class SlackNotifier:
    def send_change_notification(api_name, summary, severity):
        blocks = [
            {
                "type": "header",
                "text": {"type": "plain_text", "text": f"{emoji} {api_name}"}
            },
            {
                "type": "section",
                "text": {"type": "mrkdwn", "text": f"*Сводка:*\n{summary}"}
            },
            {
                "type": "section",
                "fields": [
                    {"type": "mrkdwn", "text": f"*API:*\n{api_name}"},
                    {"type": "mrkdwn", "text": f"*Важность:*\n{severity}"}
                ]
            }
        ]
        
        client.chat_postMessage(channel=channel, blocks=blocks)
```

**Форматирование:**
- Эмодзи по severity: 🔵 minor, 🟡 moderate, 🔴 major
- Блочная структура для лучшей читаемости
- Кнопки для быстрого доступа к документации

## 📊 Производительность

### Оптимизации

1. **Быстрое сравнение по хешу**
   - SHA-256 хеш контента
   - Если хеши равны → пропустить детальное сравнение

2. **Структурное сравнение OpenAPI**
   - DeepDiff вместо текстового сравнения
   - AI только при обнаружении изменений

3. **Кэширование в БД**
   - Индексы на url, created_at, has_changes
   - Быстрый поиск последнего снэпшота

4. **Асинхронность (будущее улучшение)**
   - Параллельная обработка нескольких URLs
   - Async/await для HTTP запросов

### Метрики

```python
# Типичное время обработки одного URL:
- Получение контента: 1-3 сек (ZenRows), 0.5-1 сек (direct)
- Сравнение по хешу: <0.01 сек
- Структурное сравнение OpenAPI: 0.1-0.5 сек
- AI анализ (OpenRouter): 2-5 сек
- Сохранение в БД: <0.1 сек
- Отправка в Slack: 0.5-1 сек

Итого: 4-10 сек на URL (с изменениями), 1-4 сек (без изменений)
```

## 🔐 Безопасность

### Хранение секретов

```bash
# .env файл (не коммитить в git!)
ZENROWS_API_KEY=secret
OPENROUTER_API_KEY=secret
SLACK_BOT_TOKEN=secret
```

### Валидация входных данных

```python
# URL валидация
if not url.startswith(('http://', 'https://')):
    raise ValueError("Invalid URL")

# Ограничение размера контента
MAX_CONTENT_SIZE = 10 * 1024 * 1024  # 10 MB
if len(content) > MAX_CONTENT_SIZE:
    raise ValueError("Content too large")
```

### Rate limiting

```python
# Для API с лимитами
import time

for url in urls:
    process_url(url)
    time.sleep(1)  # Задержка между запросами
```

## 🚀 Масштабирование

### Горизонтальное масштабирование

```python
# Разделение URLs между воркерами
urls_chunk_1 = urls[0:100]
urls_chunk_2 = urls[100:200]

# Воркер 1
python -m watcher_v2 --urls urls_chunk_1.json

# Воркер 2
python -m watcher_v2 --urls urls_chunk_2.json
```

### Использование PostgreSQL

```env
DATABASE_URL=postgresql://user:pass@localhost/api_watcher
```

**Преимущества:**
- Лучшая производительность для больших объемов
- Поддержка конкурентного доступа
- Репликация и бэкапы

### Кэширование результатов

```python
# Redis для кэширования
import redis

cache = redis.Redis(host='localhost', port=6379)

def get_cached_content(url):
    cached = cache.get(f"content:{url}")
    if cached:
        return cached.decode('utf-8')
    return None

def cache_content(url, content, ttl=3600):
    cache.setex(f"content:{url}", ttl, content)
```

## 📈 Мониторинг

### Логирование

```python
import logging

logging.basicConfig(
    level=logging.INFO,
    format='%(asctime)s - %(name)s - %(levelname)s - %(message)s',
    handlers=[
        logging.FileHandler('api_watcher.log'),
        logging.StreamHandler()
    ]
)
```

### Метрики

```python
# Собирать метрики
metrics = {
    'total_urls': len(urls),
    'changed': sum(1 for r in results if r['has_changes']),
    'errors': sum(1 for r in results if 'error' in r),
    'duration': time.time() - start_time
}

# Отправлять в систему мониторинга
send_to_prometheus(metrics)
```

---

**Архитектура спроектирована для:**
- ✅ Простоты использования
- ✅ Расширяемости
- ✅ Производительности
- ✅ Надежности
