...
Теперь используется OpenRouter для AI анализа изменений с поддержкой множества моделей.
...
- ✅ Fallback на Gemini если OpenRouter не настроен
...
Если OpenRouter не настроен, система автоматически использует Gemini:

```
⚠️ OpenRouter не настроен
✅ Gemini AI инициализирован (fallback)
```
...
3. Gemini Pro 1.5 - хорошее качество, дешевле
4. Llama 3.1 70B - приемлемое качество, очень дешево

### По скорости
1. Llama 3.1 70B - ~1-2 сек
2. Gemini Pro 1.5 - ~2-3 сек
3. Claude 3.5 Sonnet - ~3-4 сек
4. GPT-4 Turbo - ~4-6 сек

### По цене (за 1000 запросов, ~500 токенов каждый)
1. Llama 3.1 70B - ~$0.20
2. Gemini Pro 1.5 - ~$0.50
...
**Q: Можно ли использовать и Gemini и OpenRouter одновременно?**  
A: Нет, используется только один - приоритет у OpenRouter.
