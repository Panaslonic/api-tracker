# OpenRouter + Webhook - Краткая сводка

## ✅ Что сделано

### 1. OpenRouter AI вместо Gemini
- ✅ Создан `api_watcher/utils/openrouter_analyzer.py`
- ✅ Поддержка 100+ моделей (Claude, GPT-4, Llama и др.)
- ✅ Автоматический fallback на Gemini
- ✅ Обновлен `watcher_v2.py` для использования OpenRouter

### 2. Webhook уведомления
- ✅ Создан `api_watcher/notifier/webhook_notifier.py`
- ✅ Отправка уведомлений об изменениях API
- ✅ Отправка уведомлений об обновлении URL
- ✅ Еженедельные сводки
- ✅ Тест подключения при инициализации

### 3. Конфигурация
- ✅ Добавлены переменные в `config.py`:
  - `OPENROUTER_API_KEY`
  - `OPENROUTER_MODEL`
  - `OPENROUTER_SITE_URL`
  - `OPENROUTER_APP_NAME`
  - `WEBHOOK_URL`
- ✅ Обновлен `.env.example`

### 4. Документация
- ✅ `OPENROUTER_WEBHOOK_GUIDE.md` - полное руководство
- ✅ Обновлены `README.md` и `CHANGELOG.md`

## 🚀 Быстрый старт

### Настройка OpenRouter

```bash
# 1. Получите ключ на https://openrouter.ai/keys
# 2. Добавьте в .env
OPENROUTER_API_KEY=sk-or-v1-your-key-here
OPENROUTER_MODEL=anthropic/claude-3.5-sonnet
```

### Настройка Webhook

```bash
# Добавьте в .env
WEBHOOK_URL=https://your-webhook.com/api/notifications
```

### Запуск

```bash
cd api_watcher
pip install -r requirements.txt
python -m api_watcher.watcher_v2
```

### Проверка логов

```
✅ ZenRows клиент инициализирован
✅ OpenRouter AI инициализирован (модель: anthropic/claude-3.5-sonnet)
✅ Slack notifier инициализирован
✅ Webhook notifier инициализирован
✅ Webhook: тест подключения успешен (200)
```

## 📊 Рекомендуемые модели OpenRouter

| Модель | Цена | Качество | Скорость |
|--------|------|----------|----------|
| `anthropic/claude-3.5-sonnet` | $3/$15 | ⭐⭐⭐⭐⭐ | Быстро |
| `openai/gpt-4-turbo` | $10/$30 | ⭐⭐⭐⭐⭐ | Средне |
| `google/gemini-pro-1.5` | $0.35/$1.05 | ⭐⭐⭐⭐ | Быстро |
| `meta-llama/llama-3.1-70b` | $0.35/$0.40 | ⭐⭐⭐ | Очень быстро |

## 🔔 Формат webhook уведомлений

### Изменения в API
```json
{
  "event": "api_change_detected",
  "timestamp": "2024-11-20T10:30:00.000Z",
  "data": {
    "api_name": "Stripe API",
    "method_name": "Create Customer",
    "url": "https://stripe.com/docs/api/customers",
    "summary": "Добавлен новый параметр email",
    "severity": "moderate",
    "key_changes": ["Новый параметр: email"]
  }
}
```

### Обновление URL
```json
{
  "event": "documentation_url_updated",
  "timestamp": "2024-11-20T10:30:00.000Z",
  "data": {
    "api_name": "Stripe API",
    "old_url": "https://stripe.com/docs/api/old",
    "new_url": "https://stripe.com/openapi.json",
    "doc_type": "openapi"
  }
}
```

## 🔧 Архитектура

```
watcher_v2.py
    ↓
┌───────────────────────────────────────┐
│  AI Analyzer (приоритет)              │
│  1. OpenRouter (если настроен)        │
│  2. Gemini (fallback)                 │
└───────────────┬───────────────────────┘
                │
                ▼
        Анализ изменений
                │
                ▼
┌───────────────────────────────────────┐
│  Уведомления (параллельно)            │
│  - Slack (если настроен)              │
│  - Webhook (если настроен)            │
└───────────────────────────────────────┘
```

## 📁 Новые файлы

```
api_watcher/
├── utils/
│   └── openrouter_analyzer.py      # OpenRouter AI клиент
├── notifier/
│   └── webhook_notifier.py         # Webhook уведомления
└── config.py                       # Обновлена конфигурация

OPENROUTER_WEBHOOK_GUIDE.md        # Полное руководство
OPENROUTER_WEBHOOK_SUMMARY.md      # Эта сводка
```

## 🔍 Обновленные файлы

- `api_watcher/watcher_v2.py` - использование OpenRouter и webhook
- `api_watcher/config.py` - новые переменные окружения
- `api_watcher/.env.example` - примеры настройки
- `README.md` - обновлена информация
- `CHANGELOG.md` - добавлены записи

## ✨ Преимущества

### OpenRouter
- ✅ Единый API для всех моделей
- ✅ Гибкий выбор модели по цене/качеству
- ✅ Автоматический fallback
- ✅ Поддержка 100+ моделей

### Webhook
- ✅ Универсальная интеграция
- ✅ Работает с любыми системами
- ✅ Структурированные данные (JSON)
- ✅ Параллельная отправка со Slack

## 🧪 Тестирование

### Тест OpenRouter
```python
from api_watcher.utils.openrouter_analyzer import OpenRouterAnalyzer

analyzer = OpenRouterAnalyzer(
    api_key="sk-or-v1-...",
    model="anthropic/claude-3.5-sonnet"
)

result = analyzer.analyze_changes(
    "старый текст",
    "новый текст",
    "Test API"
)
print(result)
```

### Тест Webhook
```bash
curl -X POST https://your-webhook.com/api/notifications \
  -H "Content-Type: application/json" \
  -d '{"event":"test_connection","data":{"message":"Test"}}'
```

## 💡 Примеры использования

### Только OpenRouter (без webhook)
```bash
OPENROUTER_API_KEY=sk-or-v1-...
OPENROUTER_MODEL=anthropic/claude-3.5-sonnet
```

### Только Webhook (без AI)
```bash
WEBHOOK_URL=https://your-webhook.com/api/notifications
```

### Полная настройка
```bash
OPENROUTER_API_KEY=sk-or-v1-...
OPENROUTER_MODEL=anthropic/claude-3.5-sonnet
WEBHOOK_URL=https://your-webhook.com/api/notifications
SLACK_BOT_TOKEN=xoxb-...
SLACK_CHANNEL=#api-changes
```

## 📚 Документация

- 📖 [Полное руководство](OPENROUTER_WEBHOOK_GUIDE.md)
- 📖 [OpenRouter документация](https://openrouter.ai/docs)
- 📖 [Список моделей](https://openrouter.ai/models)

## ❓ FAQ

**Q: Обязательно ли использовать OpenRouter?**  
A: Нет, система автоматически использует Gemini если OpenRouter не настроен.

**Q: Можно ли использовать только webhook без Slack?**  
A: Да, просто не настраивайте SLACK_BOT_TOKEN.

**Q: Какую модель выбрать?**  
A: Рекомендуем Claude 3.5 Sonnet - лучшее соотношение цена/качество.

**Q: Что если webhook недоступен?**  
A: Система продолжит работу, ошибки будут залогированы.

**Q: Сколько стоит OpenRouter?**  
A: От $0.35 до $30 за 1M токенов в зависимости от модели.

---

**Интеграция завершена! 🎉**

Система теперь использует OpenRouter AI для анализа и отправляет уведомления на webhook + Slack.
